...
def getAction(state):
    if random.random() < BETA:
        ...
        r = getSymbolicAction(state)
        return (True,r)

def getRLAction(state):
    if random.random() < EPSILON:
        return env.action_space.sample()
    else:
        return np.argmax(q_table[state])

def getSymbolicAction(plantrace,i):
    unit = plantrace[i]
    action = unit[1]
    if action.startswith("move(n)"):
        return 1
    if action.startswith("move(s)"):
        return 0
    if action.startswith("move(e)"):
        return 2
    if action.startswith("move(w)"):
        return 3
    if action.startswith("pickup"):
        return 4
    if action.startswith("dropoff"):
        return 5
